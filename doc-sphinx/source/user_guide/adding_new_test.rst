...
Grafana. Event status logging is done automatically, when *RGT_INFLUX_TOKEN*
and *RGT_INFLUX_URI* are in the environment, and doesn't require
any information from the test. However, to log test
...
The *metrics.txt* file can be generated by the application, the job
script, the check script, or the report script.
Execution time is recorded as the time
between calling ``log_binary_execution_time.py --mode start`` and
``log_binary_execution_time.py --mode final``.
