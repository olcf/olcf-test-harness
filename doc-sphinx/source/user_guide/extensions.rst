...
In the list below, each status in *nodecheck.txt* must specify a status present in the square braces for one of the 4 common statuses.
These values are:
...
So to classify a successful test on a node, *nodecheck.txt* may use *SUCCESS*, *OK*, *GOOD*, *PASS*, or *PASSED* keywords, not case-sensitive.
...
For example, you may use a value of ``1`` to dictate a failure.
...
To enable this extension, simply create a file named *check_alias.txt* in the Run_Archive directory of a test launch (ie, */Path/to/Tests/$app/$test/Run_Archive/$test_id/check_alias.txt*).
**check_alias** is set to the content of the first line of this file.
This **check_alias** field is set in each event file, so InfluxDB is not required, but **check_alias** is sent to InfluxDB with event information, if InfluxDB is enabled.
